...
exports.registrationUser = (0, catchAsyncError_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const isEmailExist = await user_models_1.default.findOne({ email });
        if (isEmailExist) {
...
exports.activateUser = (0, catchAsyncError_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const existUser = await user_models_1.default.findOne({ email });
        if (existUser) {
            return next(new ErrorHandler_1.default("Email already exist", 400));
        }
...
exports.updateAccessToken = (0, catchAsyncError_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 259200); // 3 days 
        next();
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
...
exports.updatePassword = (0, catchAsyncError_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const user = await user_models_1.default.findById(req.user?._id).select("+password");
...
exports.updateProfilePicture = (0, catchAsyncError_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const userId = req.user?._id;
        const user = await user_models_1.default.findById(userId);
        // if user have avatar then delete the old one and upload the new one
        if (avatar && user) {
...
exports.updateUserRole = (0, catchAsyncError_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        // Ensure both email and role are provided
        if (!email || !role) {
            return next(new ErrorHandler_1.default('Email and role are required', 400));
        }
        // Call the service with email and role
        await (0, user_service_1.updateUserRoleService)(res, email, role);
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
    }
